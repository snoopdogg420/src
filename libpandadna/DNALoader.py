...
class DNALoader:
    ...
    def handleStorageData(self, dgi):
        ...
        # nodes
        self.__handleNode(dgi, target = self.curStore.storeNode)
        self.__handleNode(dgi, target = self.curStore.storeHoodNode)
        self.__handleNode(dgi, target = self.curStore.storePlaceNode)
    ...
    def __handleNode(self, dgi, target = None):
        if target is None:
            return
        ...
        for i in xrange(numNodes):
            ...
            np = NodePath(loader.loadModel(filename))
    ...
    def handleCompData(self, dgi):
        ...
        self.__debug_lp = (propCode, compClassTable.get(propCode))
