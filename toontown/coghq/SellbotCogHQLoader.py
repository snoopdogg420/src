...
class SellbotCogHQLoader(CogHQLoader.CogHQLoader):
    ...
    def loadPlaceGeom(self, zoneId):
        ...
        if zoneId == ToontownGlobals.SellbotHQ:
            ...
            dgText.setPosHpr(dgSign, 0, -0.01, 0, 0, 0, 0)
            ...
            factoryTypeText.setPosHpr(factorySign, 0, -0.01, 0, 0, 0, 0)
            ...
            factoryText.setPosHpr(factorySign, 0, -0.01, 0, 0, 0, 0)
            ...
            for door in [door0, door1, door2, door3]:
                ...
                door.setPos(door, 0, -0.1, 0)
                doorFrame.setPos(doorFrame, 0, -5, 0)
            rbc.collect()
        elif zoneId == ToontownGlobals.SellbotFactoryExt:
            rbc = RigidBodyCombiner('sbhq')
            self.geom = NodePath(rbc)
            node = loader.loadModel(self.factoryExteriorModelPath)
            factoryLinkTunnel = node.find('**/tunnel_group2')
            ...
            hqTypeText.setPosHpr(hqSign, 0, -0.01, 0, 0, 0, 0)
            ...
            hqText.setPosHpr(hqSign, 0, -0.01, 0, 0, 0, 0)
            frontDoor = node.find('**/doorway1')
            ...
            fdTypeText.setPosHpr(fdSign, 0, -0.01, 0, 0, 0, 0)
            ...
            fdText.setPosHpr(fdSign, 0, -0.01, 0, 0, 0, 0)
            sideDoor = node.find('**/doorway2')
            ...
            sdTypeText.setPosHpr(sdSign, 0, -0.01, 0, 0, 0, 0)
            ...
            sdText.setPosHpr(sdSign, 0, -0.01, 0, 0, 0, 0)
            node.reparentTo(self.geom)
            rbc.collect()
