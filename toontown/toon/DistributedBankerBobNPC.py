...
class DistributedBankerBobNPC(DistributedNPCToonBase):
    ...
    def setMovie(self, mode, npcId, avId, quests, timestamp):
        ...
        if mode == NPCToons.QUEST_MOVIE_COMPLETE:
            ...
            #Add bank interaction
            avName = base.cr.doId2do.get(avId).getName()
            self.setChatAbsolute('Greetings %s' % avName + '! Transfer all the jellybeans you need.', CFSpeech | CFTimeout)
            if base.cr.doFind('DistributedBankerMgr'):
                print 'entering the DistributedBankerMgr'
                base.cr.doFind('DistributedBankerMgr').sendUpdate('avatarEnter', [])
            else:
                print 'DistributedBankerMgr not found!'
            return
